  ...
  return (
    <div className="p-8">
      ...
      <div className="flex gap-3 mb-8 flex-wrap">
        <Link href="/portal/requests" className="flex items-center gap-2 px-4 py-2 bg-amber-500 text-slate-950 rounded-lg text-sm font-semibold hover:bg-amber-400 transition-colors">
          View Requests Inbox <ArrowRight className="w-4 h-4" />
        </Link>
        <Link href="/portal/decision-queue" className="flex items-center gap-2 px-4 py-2 bg-slate-800 text-slate-100 rounded-lg text-sm font-medium hover:bg-slate-700 transition-colors border border-slate-700">
          Decision Queue
        </Link>
        <Link href="/portal/compliance-review" className="flex items-center gap-2 px-4 py-2 bg-orange-500/15 border border-orange-500/30 text-orange-400 rounded-lg text-sm font-medium hover:bg-orange-500/25 transition-colors">
          Compliance Review
        </Link>
        <Link href="/portal/pricing-feasibility" className="flex items-center gap-2 px-4 py-2 bg-slate-800 text-slate-100 rounded-lg text-sm font-medium hover:bg-slate-700 transition-colors border border-slate-700">
          Pricing & Feasibility
        </Link>
        <Link href="/portal/clients" className="flex items-center gap-2 px-4 py-2 bg-slate-800 text-slate-100 rounded-lg text-sm font-medium hover:bg-slate-700 transition-colors border border-slate-700">
          Clients
        </Link>
        <Link href="/portal/audit-log" className="flex items-center gap-2 px-4 py-2 bg-slate-800 text-slate-100 rounded-lg text-sm font-medium hover:bg-slate-700 transition-colors border border-slate-700">
          Audit Log
        </Link>
      </div>
      ...
      <div className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
          ...
          <Link href="/portal/requests" className="text-xs text-amber-400 hover:text-amber-300 flex items-center gap-1">
            ...
            <tbody>
              {recentRequests.map((req) => (
                <tr key={req.id} className="border-b border-slate-800/50 hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <Link href={`/portal/requests/${req.id}`} className="text-xs text-amber-400 hover:text-amber-300">View →</Link>
                  </td>
                </tr>
              ))}
            </tbody>
